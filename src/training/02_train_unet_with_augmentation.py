...
WITH augmentation on TRAIN only
...
def parse_args():
    parser = argparse.ArgumentParser(description="3D U-Net (MONAI) training with augmentation (train only).")
...
def get_train_transform():
    ...
    trans = (5, 5, 1)           # voxels
    scale = (0.10, 0.10, 0.05)  # +/- in each axis
...
def main():
    ...
    train_ds = NiftiSegDataset(train_items, transform=train_transform)
    val_ds = NiftiSegDataset(val_items, transform=None)
    test_ds = NiftiSegDataset(test_items, transform=None)
