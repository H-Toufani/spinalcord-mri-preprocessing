"""
Summary:
    Laplacian-of-Gaussian (LoG) filtering on cropped spinal cord MRI volumes
...
What this script does:
    For each dataset group (multiple roots), it:
      1) Loads:
           ...
           - CROPPED image (default: T2s_CLAHE.nii.gz) where LoG is applied
      ...
      3) Computes CNR BEFORE LoG: |mu_cord - mu_surround| / sigma_bg
      4) Applies LoG filtering (scipy.ndimage.gaussian_laplace) on the cropped volume and saves output.
      5) Computes CNR AFTER LoG using the same ROIs and same sigma_bg.
      6) Exports per-case results + per-group + overall summary to an Excel file.

Requirements:
    pip install numpy nibabel scipy openpyxl

Inputs (per subject folder):
    - Original image: T2s.nii.gz (configurable via --orig_image_name)
    - Cropped image : T2s_CLAHE.nii.gz (configurable via --crop_image_name)
    - Cropped mask  : T2s_seg_cropped.nii.gz (configurable via --cord_mask_name)

Outputs:
    - LoG volume saved per subject folder (default: T2s_LoG.nii.gz)
    - Excel workbook (default: log_cnr_before_after_sigma_from_original.xlsx)
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoG filtering + CNR evaluation + Excel export.")
    ...
    parser.add_argument("--crop_image_name", type=str, default="T2s_CLAHE.nii.gz",
                        help="Cropped image filename where LoG is applied.")
    ...
    parser.add_argument("--out_name", type=str, default="T2s_laplace.nii.gz", help="Output LoG filename saved per subject.")
    parser.add_argument("--excel_out", type=str, default="log_cnr_before_after_sigma_from_original.xlsx", help="Output Excel filename.")

    # LoG parameters
    parser.add_argument("--log_sigma", type=float, default=1.0, help="LoG sigma (default: 1.0).")
    ...
    for group_name, group_root in valid_groups.items():
        for root, dirs, files in os.walk(group_root):
            if CROP_IMAGE_NAME in files:
                ...
                # Surrounding tissues ring in CROPPED space (computed on original cropped intensities)
                ...
                # ---- LoG FILTER on CROPPED image ----
                # gaussian_laplace returns the Laplacian of a Gaussian-smoothed image
                vol_log = gaussian_laplace(vol, sigma=LOG_SIGMA).astype(np.float32)

                out_path = os.path.join(root, OUT_NAME)
                nib.save(nib.Nifti1Image(vol_log, affine), out_path)

                # ---- CNR AFTER (on LoG image, same ROIs, same sigma_bg) ----
                cord_vals_a = vol_log[cord_mask]
                sur_vals_a = vol_log[surround_mask]
